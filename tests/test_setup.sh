...
# Edge cases
ENV_EMPTY=""
ENV_TIMESTAMP_ONLY="# Generated by setup.sh on Mon May 24 2026"

hash_empty=$(env_hash "$ENV_EMPTY")
hash_ts=$(env_hash "$ENV_TIMESTAMP_ONLY")

# Note: these hashes differ because echo "" produces a bare newline (non-empty
# input to md5sum) while stripping the only line from ENV_TIMESTAMP_ONLY leaves
# truly empty output. Both correctly differ from a populated .env.
if [[ "$hash_empty" != "$hash_a" ]]; then
  echo "  [PASS] empty env: hash differs from populated env"
  PASS=$(( PASS + 1 ))
else
  echo "  [FAIL] empty env: hash should differ from populated env"
  FAIL=$(( FAIL + 1 ))
fi
if [[ "$hash_ts" != "$hash_a" ]]; then
  echo "  [PASS] timestamp-only env: hash differs from populated env"
  PASS=$(( PASS + 1 ))
else
  echo "  [FAIL] timestamp-only env: hash should differ from populated env"
  FAIL=$(( FAIL + 1 ))
fi
